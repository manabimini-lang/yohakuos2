    ...
    <div className="min-h-screen bg-[#FDFDFD] dark:bg-[#0A0A0A] text-black/80 dark:text-foreground/80 pb-32 selection:bg-black/10 dark:selection:bg-white/10">
      <main className="max-w-3xl mx-auto px-6 pt-24 md:pt-32 space-y-32">
        ...
        {latestReflection ? (
          <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 fill-mode-both">
            ...
            <div className="group space-y-8">
              <div className="flex items-center gap-4 text-xs font-mono text-black/40 dark:text-foreground/40">
              ...
              <div className="pl-12 space-y-6">
                <p className="text-sm md:text-base leading-loose font-light text-black/70 dark:text-foreground/70 whitespace-pre-wrap">
                ...
                {latestReflection.status === "pending" && (
                  <p className="text-xs text-black/40 dark:text-foreground/40 italic font-light">
                    (音声を生成しています...)
                  </p>
                )}
                ...
                {latestReflection.status === "failed" && (
                  <div className="space-y-2 pt-2">
                    <p className="text-sm text-black/60 dark:text-foreground/60 font-light">
                      AIは今夜、静かに休んでいます。
                    </p>
                    <p className="text-xs text-black/40 dark:text-foreground/40 font-light leading-relaxed">
                ...
                {latestReflection.duration && (
                  <div className="text-[10px] text-black/30 dark:text-foreground/30 font-light">
            ...
            <div className="space-y-16">
              {pastReflections.map((audio, index) => (
                ...
                >
                  ...
                  <div className="pl-12 space-y-6">
                    <p className="text-sm leading-loose font-light text-black/60 dark:text-foreground/60 line-clamp-3 whitespace-pre-wrap">
                      {audio.script}
                    </p>
                    ...
                    <div className="flex items-center justify-between gap-4 text-xs">
                      ...
                      {audio.status === "pending" && (
                        <p className="text-black/30 dark:text-foreground/30 italic font-light">
                          生成中...
                        </p>
                      )}

                      {audio.duration && (
                        <div className="text-black/30 dark:text-foreground/30 font-light whitespace-nowrap">
                      ...
                      <Link
                        ...
                        className="text-black/30 dark:text-foreground/30 hover:text-black/60 dark:hover:text-foreground/60 transition-colors"
                      >
        ...
        {!isPremium && audioReflections.length >= 10 && (
          <div className="pt-16 border-t border-black/5 dark:border-border/50 text-center">
            <p className="text-xs text-black/30 dark:text-foreground/30 font-light">
              これより古い記憶は霧の中に沈んでいます。
            </p>
          </div>
        )}
        ...
        {hasAiConnection && audioReflections.length > 0 && (
          <section className="pt-24 border-t border-black/5 dark:border-border/50 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 fill-mode-both">
            <h2 className="text-xs tracking-[0.2em] uppercase text-black/40 dark:text-foreground/40">
              次の余白
            </h2>
