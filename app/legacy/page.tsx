    ...
    <div className="min-h-screen bg-white dark:bg-black text-black dark:text-white selection:bg-black/10 dark:selection:bg-white/10">
      <main className="max-w-5xl mx-auto px-6 sm:px-8 lg:px-10 pt-20 pb-24 space-y-16">
        <section className="space-y-6">
          ...
          <p className="max-w-3xl text-base sm:text-lg leading-relaxed text-black/60 dark:text-white/60 font-light">
            記録は単なる履歴ではありません。時間を重ねた空気感と、何度も戻ってきたテーマのかたちを、静かに見つめる場所です。
          </p>

          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-5 space-y-2">
            ...
            <Link
              href="/legacy/export"
              className="inline-flex items-center gap-2 rounded-full border border-black/10 dark:border-white/10 bg-black/[0.04] dark:bg-white/10 px-5 py-3 text-sm font-light text-black/70 dark:text-white/70 hover:bg-black/[0.08] dark:hover:bg-white/15 transition-colors"
            >
              スナップショットをテキストで見る
        ...
        {hasData ? (
          <div className="space-y-16">
            <section className="space-y-8">
              ...
              <div className="space-y-4">
                {legacy.timelineEntries.map((entry, index) => (
                  <div key={index} className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-6">
                    <p className="text-xs uppercase tracking-[0.24em] text-stone-500 dark:text-stone-400 mb-3">
                      {entry.label}
                    </p>
                    <h3 className="text-xl font-light text-black/85 dark:text-white/85 leading-snug">
                      {entry.title}
                    </h3>
              ...
              <section className="space-y-8">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {legacy.returningThemes.slice(0, 4).map((theme, index) => (
                    <div key={index} className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-6">
                      <h3 className="text-xl font-light text-black/85 dark:text-white/85">
                        「{theme.name}」
                      </h3>
                      <p className="mt-3 text-sm leading-relaxed text-black/60 dark:text-white/60">
            ...
            {legacy.philosophyDrift && (
              <section className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-6">
                <div className="space-y-3">
            ...
            {legacy.resonancePath.length > 0 && (
              <section className="space-y-6">
                ...
                <div className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-6">
                  <p className="text-sm leading-relaxed text-black/60 dark:text-white/60">
              ...
              <section className="space-y-8">
                ...
                <div className="grid gap-4">
                  {legacy.pastLetters.map((letter) => (
                    <div key={letter.id} className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-6">
                      <p className="text-xs uppercase tracking-[0.24em] text-stone-500 dark:text-stone-400 mb-3">
                        {letter.date}
                      </p>
                      <p className="text-sm leading-relaxed text-black/60 dark:text-white/60">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  {legacy.lifeChapters.map((chapter, index) => (
                    <div key={index} className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-6">
                      ...
                      <h3 className="text-lg font-light text-black/85 dark:text-white/85">
                        {chapter.title}
                      </h3>
                      <p className="mt-3 text-sm leading-relaxed text-black/60 dark:text-white/60">
                        {chapter.description}
          ...
          <section className="rounded-3xl border border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-white/5 p-12 text-center">
            <p className="text-lg font-light text-black/70 dark:text-white/70 leading-relaxed">
              まだ人生の軌跡は静かに育っています。
            </p>
            <p className="mt-4 text-sm font-light text-black/50 dark:text-white/50 leading-relaxed">
