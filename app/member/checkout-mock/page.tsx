...
function CheckoutMockContent() {
  ...
  return (
    <main className="min-h-screen bg-slate-50/50 flex items-center justify-center p-4 selection:bg-slate-100">
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-8">
        {/* Title */}
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-medium text-slate-800 tracking-tight pt-2">
            YOHAKU 会員プランの有効化
          </h1>
        ...
        <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100/50 space-y-4">
          <div className="flex justify-between items-center text-sm">
            <span className="text-slate-500">対象プラン</span>
            <span className="font-medium text-slate-800">YOHAKU 会員プラン (月額)</span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-slate-500">請求額 (シミュレーション)</span>
            <span className="font-semibold text-slate-900 text-base">¥980 / 月</span>
          </div>
          <div className="pt-3 border-t border-slate-200/50 text-[11px] text-slate-400 leading-relaxed">
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={isLoading}
            className="w-full rounded-2xl bg-slate-900 py-3 text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50"
          >
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-slate-50/50 flex items-center justify-center">
        <p className="text-sm text-slate-400 font-mono tracking-widest animate-pulse">読み込んでいます...</p>
      </div>
    }>
      <CheckoutMockContent />
    </Suspense>
