  ...
  return (
    <div className="space-y-6 text-slate-800">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-slate-900">セーフティ・コンソール</h1>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50 hover:text-slate-900 transition"
        >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm transition hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">監視アクティブインシデント</span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">名のアクティブ制限</span>
          </div>
        </div>

        <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm transition hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">保留中のモデレーター審査</span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">件の未完了レビュー</span>
          </div>
        </div>

        <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm transition hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">エスカレーション済件数</span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">件の管理者要対応ケース</span>
          </div>
        </div>

        <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm transition hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">正常アカウント（Safe）</span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">名の安全状態</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">安全状態の構成内訳</h2>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-slate-900">{review.user?.name || "匿名メンバー"}</p>
                    ...
                    <div className="text-[11px] text-slate-600">
                      ...
                      {review.signals.length === 0 ? (
                        <p className="text-slate-400">シグナル情報なし</p>
                      ) : (
                ...
                {data.transitions.length === 0 ? (
                  ...
                  data.transitions.map((trans) => (
                    <div key={trans.id} className="relative">
                      ...
                      <p className="text-[11px] text-slate-500 mt-0.5">{trans.reason}</p>
                    </div>
                  ))
                )}
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <form onSubmit={handleTriggerSignal} className="space-y-3.5">
              <div>
                <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">対象ユーザー</label>
                <select
                  value={simulatedUser}
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">シグナルタイプ</label>
                <select
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-[10px] font-semibold text-slate-500 uppercase">シグナル強度値</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">詳細説明</label>
                <textarea
          ...
          <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <form onSubmit={handleManualTransition} className="space-y-3.5">
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">変更後の状態</label>
                <select
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">変更の理由</label>
                <input
                  type="text"
                  value={manualReason}
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-5">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">定義されている安全ポリシー</h3>
            </div>
            <div className="space-y-3 mt-3 text-[11px] text-slate-600">
              <div className="border-l-2 border-slate-300 pl-2">
                ...
                <p className="text-slate-500">AIがユーザーの精神的依存を促す返答傾向を検知。中リスク以上で審査キュー追加。</p>
              </div>
              <div className="border-l-2 border-slate-300 pl-2">
                <p className="font-semibold text-slate-700">不適切AI生成ポリシー (policy-1002)</p>
                <p className="text-slate-500">AIによる危険または不適切な返答パターンの兆候。高リスクで制限措置の提案。</p>
              </div>
              <div className="border-l-2 border-slate-300 pl-2">
                <p className="font-semibold text-slate-700">ハラスメント対応ポリシー (policy-1003)</p>
                <p className="text-slate-500">他ユーザーやDMでの嫌がらせ・攻撃シグナル。モデレーターへのエスカレーション。</p>
              </div>
            </div>
          </div>
      ...
      {selectedReview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-sm font-bold text-slate-900">モデレーター審査裁定</h3>
                <p className="text-[10px] text-slate-400">Review ID: {selectedReview.id.slice(0, 12)}...</p>
              </div>
              <button
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-lg bg-slate-50 p-3 text-xs">
                ...
                <p className="text-slate-500 text-[10px]">Email: {selectedReview.user?.email}</p>
                <p className="mt-1.5 text-slate-500">
                  AI評価リスクスコア: <span className="font-bold text-slate-800">{(selectedReview.riskScore * 100).toFixed(0)}%</span>
                </p>
                <p className="text-slate-500">
                  推奨される安全状態: <span className="font-bold text-slate-800">{selectedReview.riskLevel}</span>
                </p>
              </div>

              <form onSubmit={handleResolveReview} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">裁定決定</label>
                  <select
                    value={decision}
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1.5">裁定理由・対応メモ</label>
                  <textarea
                    required
                    value={notes}
