  ...
  return (
    <div className="mx-auto max-w-xl pb-24 pt-4 md:pt-12 space-y-16 animate-in fade-in duration-1000">
      ...
      <section className="text-center space-y-4 px-4">
        <h1 className="text-2xl md:text-3xl font-medium text-foreground tracking-wide leading-relaxed">
          {message}
        </h1>
      ...
      <section className="px-4">
        <form onSubmit={handleSubmit} className="relative group">
          <div className="absolute -inset-1 rounded-2xl bg-slate-50 opacity-0 transition-opacity group-hover:opacity-100" />
          <div className="relative rounded-2xl border border-slate-100 bg-white p-6 md:p-8 shadow-sm transition-shadow hover:shadow-sm">
            ...
            <textarea
              ...
              className="w-full resize-none border-0 bg-transparent p-0 text-foreground placeholder:text-muted-foreground focus:ring-0 text-base md:text-lg leading-relaxed outline-none"
              rows={3}
            />
      ...
      {lastLog && (
        <section className="px-4">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-medium text-muted-foreground">前回の記録</h2>
            <span className="text-xs text-muted-foreground">{dateFmt.format(new Date(lastLog.createdAt))}</span>
          </div>
          
          <Link href="/member/ai/history" className="block">
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-colors hover:border-slate-200">
        ...
        <section className="px-4 space-y-4 pt-4 border-t border-slate-50">
          ...
          <div className="space-y-3.5">
            {discordFeed.map((post) => (
              <div 
                key={post.id} 
                className="rounded-2xl border border-slate-100 bg-white p-5 md:p-6 space-y-2.5 shadow-sm"
              >
      ...
      <section className="px-4 text-center">
        <Link 
          ...
          className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors py-4"
        >
