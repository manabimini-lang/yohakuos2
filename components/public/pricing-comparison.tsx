...
export function PricingComparison() {
  return (
    <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      <h2 className="text-lg font-medium text-slate-900">ご利用プランについて</h2>
      <div className="mt-5 overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead className="text-xs font-medium tracking-wide text-slate-400">
            <tr>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} className="border-t border-slate-50 text-slate-600">
                ...
                <td className="px-3 py-4 text-slate-400">{row.free}</td>
